...
//想要把聲音的最重要的頻率(HZ)找出來，用PitchDetector
...
SoundFile sound;//新加這一行，因為要把音閱讀近來
...
void setup() {
  ...
  sound=new SoundFile(this,"beat.mp3");//把音閱讀入
  sound.loop();
  pitchDetector = new PitchDetector(this);//新增，初始化PitchDetector物件
  //pitchDetector.input(new AudioIn(this, 0));原本的程式適用Audio in音效卡的音源輸入
